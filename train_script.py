...
class MyPlanner:
    ...
    def __init__(self):
        ...
        self.a1 = np.random.uniform(0, 10)
        self.a2 = np.random.uniform(0, 10)
        self.a3 = np.random.uniform(0, 10)
        self.a4 = np.random.uniform(0, 10)
        self.a5 = np.random.uniform(0, 10)
    ...
    def give_resource_ranking(self, df_mean_var, resource, unassigned_tasks_):
        ...
        if (df_mean_var.shape[0] > 0) & (resource in list(
                df_mean_var['resource'])):  # if df_mean_var initiated and if we have knowegle about resource
    ...
    def plan(self, available_resources, unassigned_tasks, resource_pool):
        ...
        dict_ranking_tasks = {}
        for resource in available_resources:
            dict_ranking_tasks[resource] = self.give_resource_ranking(self.df_mean_var, resource,
                                                                      set(unassigned_tasks_))
        ...
        for task in set(unassigned_tasks_):
            for resource in available_resources:
                if resource in resource_pool[task]:
                    ...
                    df_combs_score.loc[
                        curr_ind, 'tot_score'] = a1 * mean_val * +a2 * var_val + a3 * res_rank + a4 * task_rank - a5 * prob
        ...
        if (df_sched_score.shape[
            0] > 0 and curr_df_status > 2000):  # if there is at least one task resource combination in  df_sched_score
            ...
            while self.check_if_there_is_possible_match(available_resources, unassigned_tasks, resource_pool):
                for ind in range(df_sched_score.shape[0]):
                    ...
                    inds_tasks = [task_ind for task_ind in range(len(unassigned_tasks)) if
                                  (unassigned_tasks[task_ind].task_type == task)]
                    if len(inds_tasks) > 0:
    ...
    def report(self, event):
        ...
        if str(event.lifecycle_state) == 'EventType.TASK_ACTIVATE' or str(
                event.lifecycle_state) == 'EventType.COMPLETE_CASE':
            ...
            if str(event.lifecycle_state) == 'EventType.TASK_ACTIVATE':
                ...
                if prev_lifecycle == 'EventType.COMPLETE_TASK':
                    ...
                    self.df_freq_transition.loc[self.df_freq_transition['task'] == prev_task, curr_task] = \
                    self.df_freq_transition.loc[self.df_freq_transition['task'] == prev_task, curr_task].item() + 1


            elif str(event.lifecycle_state) == 'EventType.COMPLETE_CASE':
                prev_task = self.df.loc[self.df.index[-2], 'task']
                self.df_freq_transition.loc[self.df_freq_transition['task'] == prev_task, 'complete_case'] = \
                self.df_freq_transition.loc[self.df_freq_transition['task'] == prev_task, 'complete_case'].item() + 1
        ...
        if str(event.lifecycle_state) == 'EventType.COMPLETE_TASK':  # if a task just completed
            ...
            start_ind = \
            self.df.loc[(self.df['task'] == task) & (self.df['lifecycle_state'] == 'EventType.START_TASK'), :].index[-1]
            ...
            if resource in curr_resources:  # if the current resource already been added in the past
                get_ind = self.df_mean_var.loc[self.df_mean_var['resource'] == resource, :].index[
                    0]  # if so, find its row
            else:
            ...
            if get_count_value > 0:  # if the count already took place
                ...
                curr_mean = (get_count_value * get_mean_value + ser_time) / (
                            get_count_value + 1)  # compute the new service mean
                if get_count_value > 1:  # For computing variance we need two values at least
                    squer_sum = get_var_value * (
                                get_count_value - 1) + get_count_value * get_mean_value ** 2  # updating variance
                    get_count_value += 1  # updating count
                    curr_var = (squer_sum + ser_time ** 2 - get_count_value * curr_mean ** 2) / (
                                get_count_value - 1)  # updating variance
...
def single_train(my_planner, bpi_file):

    simulator = Simulator(my_planner, bpi_file)
...
def main():
    ...
    # result = simulator.run()
    my_planner = MyPlanner()
    result1 = [single_train(my_planner, "BPI Challenge 2017 - instance.pickle")[0] for ind in range(2)]
    result2 = [single_train(my_planner, "BPI Challenge 2017 - instance 2.pickle")[0] for ind in range(2)]
    print(result1, result2)
    print(np.array(result1+result2).sum())
